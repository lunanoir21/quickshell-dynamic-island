...
QtObject {
    ...
    readonly property string grpDesign: tr ? "Tasarım" : es ? "Diseño" : fr ? "Conception" : de ? "Gestaltung" : "Design"
    readonly property string grpDesignNote: tr ? "Kartın düzeni ve adanın aldığı yer"
        : es ? "La disposición de la tarjeta y el espacio que ocupa la isla"
        : fr ? "La disposition de la carte et la place que prend l'îlot"
        : de ? "Das Layout der Karte und der Platz, den die Insel einnimmt"
        : "The card's layout, and how much room the island takes"
    readonly property string grpEntrance: tr ? "Geliş" : es ? "Entrada" : fr ? "Entrée" : de ? "Einblendung" : "Entrance"
    readonly property string grpEntranceNote: tr ? "Kartın adada nasıl belirdiği"
        : es ? "Cómo aparece la tarjeta en la isla"
        : fr ? "Comment la carte apparaît dans l'îlot"
        : de ? "Wie die Karte auf der Insel erscheint"
        : "How the card arrives on the island"
    ...
    readonly property string setNotifPopup: tr ? "Bildirim kartı"
        : es ? "Tarjeta de notificación"
        : fr ? "Carte de notification"
        : de ? "Benachrichtigungskarte"
        : "Notification card"
    readonly property string setNotifPopupDesc: tr ? "Kapalıyken bildirimler sessizce geçmişe düşer"
        : es ? "Cuando está desactivado, las notificaciones solo van al historial"
        : fr ? "Désactivé, les notifications ne vont que dans l'historique"
        : de ? "Aus: Benachrichtigungen landen nur im Verlauf"
        : "Off: notifications only go to the history"
    // Entrance styles — each is named after what the eye sees, not the easing
    // curve behind it, because the picker shows the motion next to the label.
    readonly property string entranceDrop: tr ? "Düşüş" : es ? "Caída" : fr ? "Chute" : de ? "Fall" : "Drop"
    readonly property string entranceSlide: tr ? "Kayma" : es ? "Deslizar" : fr ? "Glissement" : de ? "Gleiten" : "Slide"
    readonly property string entrancePop: tr ? "Sıçrama" : es ? "Salto" : fr ? "Rebond" : de ? "Sprung" : "Pop"
    readonly property string entranceUnfold: tr ? "Açılma" : es ? "Despliegue" : fr ? "Dépliage" : de ? "Entfalten" : "Unfold"
    readonly property string entranceFade: tr ? "Sönümlenme" : es ? "Fundido" : fr ? "Fondu" : de ? "Überblenden" : "Fade"
    // Ongoing call
    readonly property string callInProgress: tr ? "Görüşme sürüyor" : es ? "Llamada en curso" : fr ? "Appel en cours" : de ? "Laufender Anruf" : "In call"
    // Button labels, so deliberately shorter than the micOn/micOff device-event
    // strings above — these sit inside a 40px control, not a card.
    readonly property string callMicOn: tr ? "Mikrofon açık" : es ? "Micrófono activo" : fr ? "Micro actif" : de ? "Mikrofon an" : "Mic on"
    readonly property string callMicOff: tr ? "Mikrofon kapalı" : es ? "Micrófono silenciado" : fr ? "Micro coupé" : de ? "Mikrofon aus" : "Mic off"
    readonly property string callHide: tr ? "Gizle" : es ? "Ocultar" : fr ? "Masquer" : de ? "Ausblenden" : "Hide"
    readonly property string callEnd: tr ? "Bitir" : es ? "Colgar" : fr ? "Raccrocher" : de ? "Beenden" : "Hang up"
    readonly property string callViewMini: tr ? "Şerit" : es ? "Barra" : fr ? "Barre" : de ? "Leiste" : "Bar"
    readonly property string callViewDetailed: tr ? "Detaylı" : es ? "Detallado" : fr ? "Détaillé" : de ? "Detailliert" : "Detailed"
    readonly property string setCallView: tr ? "Görüşme ekranı"
        : es ? "Pantalla de llamada"
        : fr ? "Écran d'appel"
        : de ? "Anrufansicht"
        : "Call screen"
    readonly property string setCallViewDesc: tr ? "Bağlanmış bir görüşmede adanın gösterdiği kadarı"
        : es ? "Cuánto muestra la isla durante una llamada conectada"
        : fr ? "Ce que l'îlot affiche pendant un appel connecté"
        : de ? "Wie viel die Insel während eines Anrufs zeigt"
        : "How much the island shows during a connected call"

    // Card designs
    readonly property string designClassic: tr ? "Klasik" : es ? "Clásico" : fr ? "Classique" : de ? "Klassisch" : "Classic"
    readonly property string designCompact: tr ? "Dar" : es ? "Compacto" : fr ? "Compact" : de ? "Kompakt" : "Compact"
    readonly property string designStacked: tr ? "Ortalı" : es ? "Centrado" : fr ? "Centré" : de ? "Zentriert" : "Stacked"
    readonly property string designRail: tr ? "Bloklu" : es ? "Bloque" : fr ? "Bloc" : de ? "Block" : "Rail"
    readonly property string designMinimal: tr ? "Sade" : es ? "Mínimo" : fr ? "Minimal" : de ? "Minimal" : "Minimal"
    readonly property string setNotifPreview: tr ? "Adada dene"
        : es ? "Probar en la isla"
        : fr ? "Tester sur l'îlot"
        : de ? "Auf der Insel testen"
        : "Try it on the island"
