...
PanelWindow {
    ...
    // swallows clicks so they never reach this one. No visible scrim — the
    // surroundings stay undimmed while the settings window is open.
    MouseArea {
        anchors.fill: parent
        enabled: settingsWin.open
                    ...
                    ColumnLayout {
                        ...
                        SettingGroup {
                            ...
                            ChoiceRow {
                                Layout.fillWidth: true
                                icon: "󰏶"
                                label: settingsWin.i18n.setCallView
                                detail: settingsWin.i18n.setCallViewDesc
                                options: [settingsWin.i18n.callViewMini,
                                          settingsWin.i18n.callViewDetailed]
                                values: ["mini", "detailed"]
                                current: settingsWin.host.callView
                                onPicked: value => {
                                    settingsWin.host.callView = value
                                    settingsWin.host.saveSettings()
                                }
                            }
                        ...
                        SettingGroup {
                            ...
                            label: settingsWin.i18n.grpDesign
                            note: settingsWin.i18n.grpDesignNote

                            SettingRow {
                                Layout.fillWidth: true
                                icon: "󰂚"
                                label: settingsWin.i18n.setNotifPopup
                                detail: settingsWin.i18n.setNotifPopupDesc
                                checked: settingsWin.host.notificationPopup
                                onToggled: {
                                    settingsWin.host.notificationPopup = !settingsWin.host.notificationPopup
                                    settingsWin.host.saveSettings()
                                }
                            }

                            NotificationDesignPicker {
                                Layout.fillWidth: true
                                opacity: settingsWin.host.notificationPopup ? 1 : 0.4
                                enabled: settingsWin.host.notificationPopup
                        ...
                        SettingGroup {
                            visible: settingsWin.section === "notifications"
                            label: settingsWin.i18n.grpEntrance
                            note: settingsWin.i18n.grpEntranceNote

                            NotificationEntrancePicker {
                                Layout.fillWidth: true
                                // Nothing to choose between while the card is
                            ...
                            // The picker's miniature only carries the motion.
                            // Seeing it at full size, on the island, with the
                            // real card, is a different judgement — and the
                            // settings window sits below the island, so it
                            // stays visible while the sample plays.
                            Rectangle {
                                id: testNotifBtn
                                Layout.fillWidth: true
                                implicitHeight: 52
                                ...
                                color: testNotifHit.containsMouse ? settingsWin.fixedChipHover : settingsWin.fixedChip
                                Behavior on color { ColorAnimation { duration: 160; easing.type: Easing.OutCubic } }
                                Behavior on opacity { NumberAnimation { duration: 160; easing.type: Easing.OutCubic } }
                                ...
                                RowLayout {
                                    ...
                                    Text {
                                        Layout.fillWidth: true
                                        text: settingsWin.i18n.setNotifPreview
                                        color: settingsWin.fixedText
                                        font.family: settingsWin.host.uiFont
                                        font.pixelSize: 15
                                ...
                                MouseArea {
                                    ...
                                    onClicked: settingsWin.host.showNotification(
                                        settingsWin.i18n.notification,
                                        settingsWin.i18n.newNotification,
                                        settingsWin.i18n.emptyNotification, "")
                                }
                            }
                        }
    ...
    // The five card designs, each drawn as a small true-to-shape maquette:
    // same logo position, same text weights, same proportions, just smaller.
    // These differ in layout rather than in motion, so unlike the entrance
    // picker they say everything standing still.
    component NotificationDesignPicker: Rectangle {
        implicitHeight: 150
        radius: 16
        color: settingsWin.fixedChip
        ...
        RowLayout {
            ...
            Repeater {
                model: settingsWin.host.notificationLayouts

                Rectangle {
                    id: designOpt
                    required property string modelData

                    readonly property bool active: settingsWin.host.notificationLayout === designOpt.modelData
                    readonly property string displayLabel: {
                        switch (designOpt.modelData) {
                        case "compact": return settingsWin.i18n.designCompact
                        case "stacked": return settingsWin.i18n.designStacked
                        case "rail": return settingsWin.i18n.designRail
                        case "minimal": return settingsWin.i18n.designMinimal
                        default: return settingsWin.i18n.designClassic
                        }
                    }
                    ...
                    color: settingsWin.fixedChipHover
                    border.width: 1
                    border.color: designOpt.active ? settingsWin.fixedText : "transparent"
                    Behavior on border.color { ColorAnimation { duration: 140 } }
                    ...
                    ColumnLayout {
                        ...
                        // The maquette. Its own height follows the design's
                        // real height, so the compact card looks like the
                        // cheap one and stacked looks like the expensive one
                        // before either is ever chosen.
                        Item {
                            Layout.fillWidth: true
                            Layout.fillHeight: true
                            ...
                            Rectangle {
                                ...
                                height: Math.min(parent.height,
                                    parent.height * settingsWin.host.notificationCardHeight(designOpt.modelData) / 172)
                                radius: 7
                                color: settingsWin.fixedSurface

                                // ---- classic / compact: logo left, lines right
                                RowLayout {
                                    anchors.fill: parent
                                    anchors.margins: 6
                                    spacing: 5
                                    visible: designOpt.modelData === "classic"
                                             || designOpt.modelData === "compact"

                                    Rectangle {
                                        Layout.alignment: Qt.AlignVCenter
                                        Layout.preferredWidth: designOpt.modelData === "compact" ? 12 : 17
                                        Layout.preferredHeight: designOpt.modelData === "compact" ? 12 : 17
                                        radius: 5
                                        color: settingsWin.fixedSurfaceAlt
                                        border.width: 1
                                        border.color: settingsWin.fixedLine
                                ...
                                // ---- stacked: everything centred, logo on top
                                ColumnLayout {
                                    anchors.centerIn: parent
                                    width: parent.width - 16
                                    spacing: 4
                                    visible: designOpt.modelData === "stacked"
                                ...
                                // ---- rail: a filled block down the left edge
                                RowLayout {
                                    anchors.fill: parent
                                    anchors.margins: 6
                                    spacing: 6
                                    visible: designOpt.modelData === "rail"
                                ...
                                // ---- minimal: a hairline and the words
                                RowLayout {
                                    anchors.fill: parent
                                    anchors.margins: 6
                                    spacing: 6
                                    visible: designOpt.modelData === "minimal"
                        ...
                        Text {
                            Layout.fillWidth: true
                            horizontalAlignment: Text.AlignHCenter
                            text: designOpt.displayLabel
                            color: designOpt.active ? settingsWin.fixedText : settingsWin.fixedMuted
                            font.family: settingsWin.host.uiFont
                            font.weight: Font.DemiBold
                            font.pixelSize: 11
                            elide: Text.ElideRight
                    ...
                    MouseArea {
                        ...
                        hoverEnabled: true
                        onClicked: {
                            settingsWin.host.notificationLayout = designOpt.modelData
                            settingsWin.host.saveSettings()
                        }
                    }
                }
    ...
    // The five notification entrances, each running its own real curve on a
    // loop inside a miniature island. A still frame of a transform says
    // nothing about how it feels, and the difference between these options is
    // nothing but motion — so the picker has to move.
    component NotificationEntrancePicker: Rectangle {
        implicitHeight: 152
        radius: 16
        color: settingsWin.fixedChip
        ...
        RowLayout {
            ...
            Repeater {
                model: settingsWin.host.notificationEntrances
                ...
                Rectangle {
                    ...
                    readonly property bool active: settingsWin.host.notificationEntrance === entranceOpt.modelData
                    readonly property string displayLabel: {
                        switch (entranceOpt.modelData) {
                        case "slide": return settingsWin.i18n.entranceSlide
                        case "pop": return settingsWin.i18n.entrancePop
                        case "unfold": return settingsWin.i18n.entranceUnfold
                        case "fade": return settingsWin.i18n.entranceFade
                        default: return settingsWin.i18n.entranceDrop
                        }
                    }

                    // Mirror the island's own two drivers, so a preview shows
                    // the contents lagging exactly as far behind as they will
                    // on the real card.
                    property real phase: 1
                    property real contentPhase: 1
                    ...
                    color: settingsWin.fixedChipHover
                    border.width: 1
                    border.color: entranceOpt.active ? settingsWin.fixedText : "transparent"
                    Behavior on border.color { ColorAnimation { duration: 140 } }

                    // The five loops are deliberately out of step with each
                    // other. Started together they spend most of every cycle
                    // resting in the same position at the same time, which is
                    // exactly when five different motions look like five
                    // identical boxes.
                    SequentialAnimation {
                        id: previewLoop
                        running: entranceOpt.visible
                        loops: Animation.Infinite

                        PauseAnimation { duration: entranceOpt.index * 190 }
                        ...
                        ParallelAnimation {
                            NumberAnimation {
                                ...
                                from: 0
                                to: 1
                                duration: settingsWin.host.entranceDuration(entranceOpt.modelData)
                                easing.type: settingsWin.host.entranceEasing(entranceOpt.modelData)
                                easing.overshoot: settingsWin.host.entranceOvershoot(entranceOpt.modelData)
                            }
                            SequentialAnimation {
                                PropertyAction { target: entranceOpt; property: "contentPhase"; value: 0 }
                                PauseAnimation {
                                    duration: settingsWin.host.entranceContentDelay(entranceOpt.modelData)
                                }
                                NumberAnimation {
                                    target: entranceOpt
                        ...
                        // Long enough to read the settled card, short enough
                        // that the box is never sitting still for long.
                        PauseAnimation { duration: 520 }
                    }

                    // A loop stopped mid-flight leaves the card frozen wherever
                    // it was; parking both drivers at rest keeps a section that
                    // was scrolled away from coming back half-animated.
                    Connections {
                        target: previewLoop
                        function onRunningChanged() {
                            if (!previewLoop.running) {
                                entranceOpt.phase = 1
                                entranceOpt.contentPhase = 1
                            }
                        }
                    }
                    ...
                    ColumnLayout {
                        ...
                        // The stage the miniature card arrives on. Clipped like
                        // the real island, so drop and unfold are seen coming
                        // out from behind its edge rather than floating in.
                        Rectangle {
                            id: miniStage
                            Layout.fillWidth: true
                            Layout.fillHeight: true
                            ...
                            // Every number below is the island's own figure
                            // scaled to this box — the miniature is about a
                            // quarter the width of the real card, so the same
                            // pixel offsets would be invisible here.
                            Rectangle {
                                id: miniCard
                                anchors.centerIn: parent
                                // Deliberately smaller than the stage: a card
                                // that fills its box has nowhere to travel, and
                                // every style then reads as the same flicker.
                                width: parent.width - 26
                                height: 27
                                radius: 7
                                color: settingsWin.fixedSurfaceAlt
                                opacity: 0.2 + 0.8 * Math.max(0, Math.min(1, entranceOpt.phase))

                                transform: [
                                    Scale {
                                        origin.x: miniCard.width / 2
                                        origin.y: entranceOpt.modelData === "unfold"
                                            ? 0 : miniCard.height / 2
                                        xScale: {
                                            switch (entranceOpt.modelData) {
                                            case "slide": return 1 + (1 - entranceOpt.phase) * 0.12
                                            case "pop": return 0.62 + 0.38 * entranceOpt.phase
                                            case "fade": return 0.97 + 0.03 * entranceOpt.phase
                                            case "drop": return 1 + Math.max(0, entranceOpt.phase - 1) * 0.55
                                            default: return 1
                                            }
                                        }
                                        yScale: {
                                            ...
                                            case "pop": return 0.62 + 0.38 * entranceOpt.phase
                                            case "fade": return 0.97 + 0.03 * entranceOpt.phase
                                            case "unfold": return Math.max(0, entranceOpt.phase)
                                            case "drop": return 1 + (1 - entranceOpt.phase) * 0.14
                                            default: return 1
                                            }
                                        }
                                    },
                                    Translate {
                                        x: entranceOpt.modelData === "slide"
                                            ? (1 - entranceOpt.phase) * 30 : 0
                                        y: entranceOpt.modelData === "drop"
                                            ? (entranceOpt.phase - 1) * 26 : 0
                                    }
                                ]
                                ...
                                RowLayout {
                                    ...
                                    opacity: entranceOpt.modelData === "fade" ? 1 : entranceOpt.contentPhase
                                    transform: Translate {
                                        x: entranceOpt.modelData === "slide"
                                            ? (1 - entranceOpt.contentPhase) * 15 : 0
                                        y: entranceOpt.modelData === "drop"
                                            ? (1 - entranceOpt.contentPhase) * -8 : 0
                                    }
                        ...
                        Text {
                            Layout.fillWidth: true
                            horizontalAlignment: Text.AlignHCenter
                            text: entranceOpt.displayLabel
                            color: entranceOpt.active ? settingsWin.fixedText : settingsWin.fixedMuted
                            font.family: settingsWin.host.uiFont
                            font.weight: Font.DemiBold
                            font.pixelSize: 11
                            elide: Text.ElideRight
                    ...
                    MouseArea {
                        ...
                        hoverEnabled: true
                        onClicked: {
                            settingsWin.host.notificationEntrance = entranceOpt.modelData
                            settingsWin.host.saveSettings()
                        }
                    }
                }
